...
import { Briefcase, Download, FileText, User } from "lucide-react";

type NavItem =
  | { type: "route"; to: "/" | "/sobre" | "/cases"; icon: typeof FileText; label: string }
  | { type: "download"; href: string; icon: typeof FileText; label: string; filename: string };

const items: NavItem[] = [
  { type: "route", to: "/", icon: FileText, label: "Currículo" },
  { type: "route", to: "/cases", icon: Briefcase, label: "Cases" },
...
export function SiteNav() {
  ...
  return (
    <nav className="fixed top-4 left-1/2 z-50 w-full max-w-lg -translate-x-1/2 px-3">
      <div
        ref={containerRef}
        ...
        {items.map((item, index) => {
          ...
          return (
            <button
              ...
              onClick={handleClick}
              className={`relative z-10 flex flex-1 flex-col items-center justify-center px-2 py-1.5 text-sm font-medium transition-colors ${
